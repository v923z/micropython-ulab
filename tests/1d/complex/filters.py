...
#from numpy import (atleast_1d, poly, np.polyval, roots, real, asarray,
#                   resize, np.pi, absolute, logspace, r_, sqrt, math.tan, log10,
...
def findfreqs(num, den, N, kind='ba'):
    ...
    """
    if kind == 'ba':
        ep = np.atleast_1d(roots(den)) + 0j
        tz = np.atleast_1d(roots(num)) + 0j
    elif kind == 'zp':
        ep = np.atleast_1d(den) + 0j
        tz = np.atleast_1d(num) + 0j
    else:
        raise ValueError("input must be one of {'ba', 'zp'}")

    if len(ep) == 0:
        ep = np.atleast_1d(-1000) + 0j
...
def freqs_zpk(z, p, k, worN=200):
    ...
    w = atleast_1d(w)  #TODO
    s = 1j * w
    num = polyvalfromroots(s, z) #TODO
    den = polyvalfromroots(s, p)
    h = k * num/den
    return w, h


def freqz(b, a=1, worN=512, whole=False, plot=None, fs=2*np.pi, include_nyquist=False):
    """
    ...
    fs : float, optional
        The sampling frequency of the digital system. Defaults to 2*np.pi
        radians/sample (so w is from 0 to np.pi).
    ...
    w : ndarray
        ...
        By default, `w` is normalized to the range [0, np.pi) (radians/sample).
    h : ndarray
    ...
    if h is None:  # still need to compute using freqs w
        zm1 = math.exp(-1j * w)
        h = (np.polyval(zm1, b, tensor=False) /
             np.polyval(zm1, a, tensor=False))
...
def freqz_zpk(z, p, k, worN=512, whole=False, fs=2*np.pi):
    r"""
...
def group_delay(system, w=512, whole=False, fs=2*np.pi):
    ...
    w : ndarray
        ...
        as `fs`.  By default, `w` is normalized to the range [0, np.pi)
        (radians/sample).
    gd : ndarray
        The group delay.
    ...
    if _is_int_type(w):
        if whole:
            w = np.linspace(0, 2 * np.pi, w, endpoint=False)
        else:
            w = np.linspace(0, np.pi, w, endpoint=False)
    else:
        w = np.atleast_1d(w)
        w = 2*np.pi*w/fs
    ...
    if not analog:
        if numpy.any(Wn <= 0) or numpy.any(Wn >= 1):
            ...
            raise ValueError("Digital filter critical frequencies "
                             "must be 0 < Wn < 1")
        fs = 2.0
        warped = 2 * fs * math.tan(np.pi * Wn / fs)
    else:
        warped = Wn
...
def ellipord(wp, ws, gpass, gstop, analog=False, fs=None):
    ...
    ord = int(math.ceil(d0[0] * d1[1] / (d0[1] * d1[0])))

    if not analog:
        wn = np.arctan2(passb) * 2.0 / np.pi
    else:
        wn = passb
...
def cheb2ap(N, rs):
    ...
    z = -conjugate(1j / sin(m * np.pi / (2.0 * N)))

    # Poles around the unit circle like Butterworth
    p = -exp(1j * np.pi * numpy.arange(-N+1, N, 2) / (2 * N))
...
def _design_notch_peak_filter(w0, Q, ftype, fs=2.0):
    ...
    if ftype == "notch":
        ...
        beta = (np.sqrt(1.0-gb**2.0)/gb)*np.math.tan(bw/2.0)
    elif ftype == "peak":
        # Compute beta: formula 11.3.19 (p.579) from reference [1]
        beta = (gb/np.sqrt(1.0-gb**2.0))*np.math.tan(bw/2.0)
    else:
...
def iircomb(w0, Q, ftype='notch', fs=2.0):
    ...
    beta = np.sqrt((GB**2 - G0**2) / (G**2 - GB**2)) * np.math.tan(N * w_delta / 4)
